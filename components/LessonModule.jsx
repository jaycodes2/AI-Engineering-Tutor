...
const LessonModule = ({ lesson, image, isImageLoading }) => {
  return (
    <div className="bg-card rounded-xl border border-border p-6 sm:p-8 animate-fade-in">
      ...
      <div className="w-full aspect-video bg-muted/50 rounded-lg mb-6 border border-border flex items-center justify-center overflow-hidden">
        {isImageLoading && <div className="text-muted-foreground animate-pulse-subtle text-sm">Generating visual aid...</div>}
        {image && !isImageLoading && (
          <img src={image} alt={lesson.title} className="w-full h-full object-cover rounded-lg animate-fade-in"/>
        )}
      </div>
