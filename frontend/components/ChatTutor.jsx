...
import { SendIcon, BrainCircuitIcon, PlusIcon } from './icons.jsx';
...
const ChatTutor = ({
    ...
    return (
        <div className="bg-card rounded-xl border border-border flex h-full shadow-sm">
            {/* Sidebar for Chat History */}
            <div className="w-1/3 border-r border-border flex flex-col bg-background">
                <div className="p-3 border-b border-border">
                    <button
                        onClick={onNewSession}
                        className="w-full flex items-center justify-center p-2 rounded-lg text-sm font-semibold bg-secondary text-secondary-foreground hover:bg-muted transition-colors"
                    >
                    ...
                    {sessions.map(session => (
                        <div
                            ...
                            className={`p-3 rounded-lg cursor-pointer text-sm truncate transition-colors ${
                                activeSessionId === session.id ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 font-semibold' : 'hover:bg-muted text-muted-foreground'
                            }`}
                        >
                            {session.title}
                        </div>
                ...
                <div ref={chatContainerRef} className="flex-grow p-4 space-y-6 overflow-y-auto">
                    {activeSession?.messages.map((msg, index) => (
                        <div key={index} className={`flex items-start gap-3 text-sm ${msg.role === 'user' ? 'justify-end' : ''}`}>
                            ...
                            <div className={`max-w-md p-3 rounded-xl shadow-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-secondary text-foreground rounded-bl-none'}`}>
